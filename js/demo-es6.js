...
$(function () {
    ...
    const server_path = "/netdisk", base_path = "/netdisk/files";

    const $btns = {  // Buttons definition.
        ...
        $fileTable = $("#fileTable"),
        $modals = {  // Modals and modal's element definition.
            upload: {
                el: $("#uploadModal"),
                file: $("#file")
            },
    ...
    /*** APIs ***/
    const $apis = {
        getFileListAjax: () => $.getJSON("data/file/file-list"),
        getFileList: () => JSON.parse($stor.get("nd_file_list")),
        setFileList: (list) => $stor.store("nd_file_list", JSON.stringify(list))
    };
    ...
    const $file = {
        ...
        loadFileList: (list, parent_id) => {
            ...
            $fileTable.bootstrapTable({
                ...
                columns: [{
                    ...
                    field: "file_size",
                    title: "大小",
                    formatter: (value) => value !== null ? getFileSize(value) : value
                }, {
                    class: "text-small",
            ...
            $fileTable.find("a.file-column.folder").on("click", function() {
                let file = $file.find($apis.getFileList(), parseInt($(this).attr("data-file-id")));
                let parent_id = $file.currentFolder;
            ...
            $fileTable.find("a.file-column.image").on("click", function() {
                const file = $file.find($apis.getFileList(), parseInt($(this).attr("data-file-id")));
        ...
        find: (arr, target_id) => {
            ...
            while (stack.length !== 0) {
                ...
                if (curFile["is_folder"]) {
                    for (let i in curFile["content_inside"]) {
                        if (!curFile["content_inside"].hasOwnProperty(i)) continue;
                        // Push all files under current path to stack.
                        stack.push({ parent: curFile, index: i, node: curFile["content_inside"][i]});
                    }
                }
            }
            // Nothing found.
        ...
        add: (target_id, file) => {
            let arr = $apis.getFileList();
            let obj;
            if (target_id !== 0) {  // Handle for root directory.
        ...
        remove: (target_id) => {
            let arr = $apis.getFileList();
            let obj = $file.find(arr, target_id);
            if (obj === null) {
                // Nothing found.
                return;
            }
            let obj_parent = obj["parent"];
            if (obj["parent"] === -1) {  // Handle for root directory.
    ...
    const $breadcrumb = {
        ...
        add: (name, file_id, parent_id) => {
            let $li = $(`<li class="node inside" data-file-id="${file_id}" data-parent-id="${parent_id}"><a>${name}</a></li>`);
        ...
        backToParent: {
            el: $(".path-previous"),
            hide: () => $breadcrumb.backToParent.el.removeClass("active"),
            show: () => $breadcrumb.backToParent.el.addClass("active"),
            method: (request_id) => {
                let file = {}, list;
                if (request_id === 0) {
                    file["parent_id"] = -1;
    ...
    $modals.addFolder.input
        .on("keydown", function(event) {  // Only works on PC.
    ...
    $modals.addFolder.save
        .on("click", function() {
            const filenameReg = /^(?!\.)[^\\\/:*?"<>|]{1,255}$/,
                filename = $modals.addFolder.input.val();
            if (filenameReg.test(filename)) {
                const folder = $file.get.folder(filename);
                $file.add($file.currentFolder, folder);
                $modals.addFolder.el.modal("hide");
                swal("创建成功", "已成功创建文件夹", "success")
                    .then(() => {
                        if (!$file.currentFolder) {
                            $file.loadFileList($apis.getFileList(), -1);
                        } else {
                            const file = $file.find($apis.getFileList(), $file.currentFolder);
    ...
    $modals.preview.el
        .on("hide.bs.modal", function() {
            $(this).find(".preview-media").each(function() {this.pause()});
    ...
    $btns.deleteFile.on("click", function() {
        const files = $fileTable.bootstrapTable("getSelections");
        swal({
            title: "删除确认",
            text: "确定删除选中文件吗？",
            icon: "warning",
    ...
    $btns.simulateUpload.on("click", function() {
        if ($modals.upload.file[0].files.length) {
            const file = $modals.upload.file[0].files[0];
            const file_main_type = file.type.substr(0,5);
            let type;
            if (file.type === "") {
                type = 0
            ...
            const new_file = $file.get.file(file.name, type, file.size);
            $file.add($file.currentFolder, new_file);
            swal("上传成功", "已成功上传文件", "success")
                .then(() => {
